...
/**
 * 自定义的XMarkdown处理器，支持表格转换为HTML
 * 
 * @author <a href="xiaoymin@foxmail.com">xiaoymin@foxmail.com</a>
 * @since torchv_server
 */
@Slf4j
public class XMarkdownWithHtmlTableContentHandler extends SafeContentHandler {
     ...
     */
    public static final Set<String> END_LINE = unmodifiableSet("p", "h1", "h2", "h3", "h4", "h5", "h6", "div", "ul",
            "ol", "dl", "pre",
     ...
     */
    private static final Set<String> HEAD = unmodifiableSet("title");
    ...
    public XMarkdownWithHtmlTableContentHandler(ContentHandler handler, Metadata metadata) {
        super(handler);
    ...
    public void setCurrentDocument(XWPFDocument document) {
        // 如果底层的handler是我们的自定义表格处理器，也要设置文档
        if (delegateHandler instanceof MarkdownWithTableHandler) {
            ((MarkdownWithTableHandler) delegateHandler).setCurrentDocument(document);
        }
    }
    ...
    private void lazyStartHead() throws SAXException {
        if (!headStarted) {
            ...
            if (lang != null) {
                ...
                htmlAttrs.addAttribute("", "xml:lang", "xml:lang", "CDATA", lang);
            }
    ...
    private void lazyEndHead(boolean isFrameset) throws SAXException {
        ...
        if (!headEnded) {
            ...
            for (String name : metadata.names()) {
                if (!TikaCoreProperties.TITLE.getName().equals(name) && !HEAD.contains(name)) {
                    super.startElement(XHTML, "meta", "meta",
                            new AttributesImpl() {
                                
                                {
                                    addAttribute("", "name", "name", "CDATA", name);
                                    addAttribute("", "content", "content", "CDATA", metadata.get(name));
                                }
                            });
